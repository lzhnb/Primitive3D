...
PRIM_HOST_DEVICE inline void project(Vector3f points[N_POINTS], const Vector3f &axis, float &min,
                                     float &max) {
...
struct BoundingBox {
    ...
    PRIM_HOST_DEVICE Vector3f diag() const {
        return max - min;
    }
    ...
    PRIM_HOST_DEVICE Vector3f center() const {
        return (max + min) * 0.5f;
    }
    ...
    PRIM_HOST_DEVICE float distance(const Vector3f &p) const {
