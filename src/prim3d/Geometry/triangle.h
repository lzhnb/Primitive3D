...
namespace prim3d {
struct Triangle {
    PRIM_HOST_DEVICE Vector3f normal() const {
        return (b - a).cross(c - a).normalized();
    }

    // based on https://www.iquilezles.org/www/articles/intersectors/intersectors.htm
    PRIM_HOST_DEVICE float ray_intersect(const Vector3f &ro, const Vector3f &rd,
                                         Vector3f &n) const {
    ...
    PRIM_HOST_DEVICE Vector3f centroid() const {
        return (a + b + c) / 3.0f;
    }
